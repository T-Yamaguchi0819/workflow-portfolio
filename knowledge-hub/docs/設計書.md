...
- **CD**(GitHub Actions `deploy.yml`): main / `ai/**` への push(backend/infra 変更時)で テスト → `sam deploy` → API 疎通確認を自動実行。ブランチで環境を振り分ける: `main` → prod(スタック `knowledge-hub`)、`ai/**` → 共有 dev(スタック `knowledge-hub-dev`)。それ以外の ref は環境解決ステップで失敗しデプロイに進まない(フェイルクローズ)。concurrency は prod/dev を別グループとし相互に干渉しない(dev グループ内は直列・後勝ち)。認証は **OIDC**(`infra/github-oidc.yaml`)— 対象リポジトリの `main`・`ai/*` ブランチからのみ AssumeRole 可能で、長期アクセスキーを GitHub に保存しない
- **dev 環境(共有・常設)**: リソース物理名は `${AWS::StackName}` ベースとし、同一テンプレートから prod(`knowledge-hub-articles` 等)と dev(`knowledge-hub-dev-articles` 等)を衝突なく生成する(prod の解決結果は従来の固定名と同一でリソース置換なし)。dev の `CorsAllowedOrigin` はテンプレートデフォルトのまま(API 呼び出しは SSR 経由のみでブラウザ直呼びなし)。フロントエンドは Amplify の自動ブランチ作成(パターン `ai/*`・`ai/**`)がブランチごとに環境を作成し、ブランチ用環境変数 `API_BASE_URL` は dev API を向く(main はアプリレベルの prod URL のまま)。ブランチ削除でブランチ環境は自動削除され、dev スタックは常設で残る。設定手順は [README のデプロイ節](../README.md#aws-へのデプロイ)に記録
- **バックエンド(手動時)**: `mvn package`(shade で Lambda 用 jar 生成)→ `sam deploy --profile portfolio`(prod)/`sam deploy --config-env dev --profile portfolio`(dev)。API URL は Stack Output `ApiEndpoint`
