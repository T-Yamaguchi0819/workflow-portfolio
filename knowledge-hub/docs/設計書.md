...
- **CI**(GitHub Actions): backend `mvn verify` / frontend `typecheck`+`build` / `sam validate --lint` の 3 ジョブを push/PR で実行
- **バックエンド**: `mvn package`(shade で Lambda 用 jar 生成)→ `sam deploy`。API URL は Stack Output `ApiEndpoint`
